/*
  Warnings:

  - You are about to drop the `kanji.codepoint` table. If the table is not empty, all the data it contains will be lost.
  - You are about to drop the column `kwSkipMisclass_id` on the `kanji_querycode` table. All the data in the column will be lost.

*/
-- DropTable
PRAGMA foreign_keys=off;
DROP TABLE "kanji.codepoint";
PRAGMA foreign_keys=on;

-- CreateTable
CREATE TABLE "kanji_codepoint" (
    "kanji_id" INTEGER NOT NULL,
    "kw_codepoint_id" INTEGER NOT NULL,
    "value" TEXT NOT NULL,

    PRIMARY KEY ("kanji_id", "kw_codepoint_id"),
    CONSTRAINT "kanji_codepoint_kanji_id_fkey" FOREIGN KEY ("kanji_id") REFERENCES "kanji" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT "kanji_codepoint_kw_codepoint_id_fkey" FOREIGN KEY ("kw_codepoint_id") REFERENCES "kw_kanji_codepointtype" ("id") ON DELETE CASCADE ON UPDATE CASCADE
);

-- RedefineTables
PRAGMA foreign_keys=OFF;
CREATE TABLE "new_kanji_querycode" (
    "kanji_id" INTEGER NOT NULL,
    "kwQueryCodeType_id" INTEGER NOT NULL,
    "value" TEXT NOT NULL,

    PRIMARY KEY ("kanji_id", "kwQueryCodeType_id"),
    CONSTRAINT "kanji_querycode_kanji_id_fkey" FOREIGN KEY ("kanji_id") REFERENCES "kanji" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT "kanji_querycode_kwQueryCodeType_id_fkey" FOREIGN KEY ("kwQueryCodeType_id") REFERENCES "kw_kanji_querycodetype" ("id") ON DELETE CASCADE ON UPDATE CASCADE
);
INSERT INTO "new_kanji_querycode" ("kanji_id", "kwQueryCodeType_id", "value") SELECT "kanji_id", "kwQueryCodeType_id", "value" FROM "kanji_querycode";
DROP TABLE "kanji_querycode";
ALTER TABLE "new_kanji_querycode" RENAME TO "kanji_querycode";
PRAGMA foreign_key_check;
PRAGMA foreign_keys=ON;
