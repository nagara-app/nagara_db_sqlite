...
export default (): Kanji[] => {
  ...
  for (const kd2El of kd2Entries) {
    const kd2Readings = toArrayOrUndefined(kd2El.reading_meaning?.rmgroup?.reading);
    const kd2Nanoris = toArrayOrUndefined(kd2El.reading_meaning?.nanori);
    const kd2Meanings = toArrayOrUndefined(kd2El.reading_meaning?.rmgroup?.meaning);
...
const createMeanings = (kd2Meanings?: Array<string | Kanjidic2CharRdngMngGrpMng>): string[] | undefined => {
...
const getStrokecount = (kd2Strokecount: string[]): number => {
  ...
  if (strokecountString === undefined) throw new Error('Kanji stroke count array is empty');
...
const getGrade = (kd2Grade: Kanjidic2MiscGrade | undefined): number | undefined => {
...
const getRadicals = (literal: string): string[] | undefined => {
  const kradfilex = fileManager.getKradfilex();
  ...
  const radicals = filteredRadicals.map((kradical) => {
    const tkdbRadical = tkdbRadicals.find((tkdbRadical) => tkdbRadical.kradical === kradical);

    if (tkdbRadical === undefined) {
      throw new Error(`kradical ${kradical} of literal ${literal} does not exist in TKDB radical`);
    }
...
export const extractXYfromPath = (path: string): { x: string; y: string } => {
  ...
  if (match !== null && match.length > 2 && match[1] !== undefined && match[2] !== undefined) {
...
const getComposition = (literal: string): KanjiComposition[] | undefined => {
  ...
  const getSubcomp = (groups: KVGKanjiGroup[] | undefined): KanjiComposition[] | undefined => {
    if (groups === undefined) {
      return undefined;
    }
    ...
    for (const group of groups) {
      ...
      const isRadical = tkdbRadicals.find((radical) => radical.radical === element) !== undefined ? true : undefined;
      ...
      const isKanji = kanjidic2.character.find((kanji) => kanji.literal === element) !== undefined ? true : undefined;
